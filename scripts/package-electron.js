...
async function packageElectron() {
  try {
    ...
    console.log('\nStep 2: Running post-build...');
    execSync('node scripts/post-build.js', { stdio: 'inherit' });

    console.log('\nStep 3: Packaging with electron-packager...');
    execSync(
      'electron-packager . CoffeeCube --platform=win32 --arch=x64 --out=dist --overwrite --ignore="^/(dist|.git|.next(?!/standalone))" --prune=false',
      { stdio: 'inherit' }
    );

    console.log('\nStep 4: Copying .next/standalone to packaged app...');
    const rootDir = path.join(__dirname, '..');
    const standaloneSource = path.join(rootDir, '.next', 'standalone');
    const packagedAppPath = path.join(rootDir, 'dist', 'CoffeeCube-win32-x64', 'resources', 'app');
    const standaloneTarget = path.join(packagedAppPath, '.next', 'standalone');

    if (!fs.existsSync(standaloneSource)) {
      throw new Error('.next/standalone folder not found. Make sure Next.js build completed successfully.');
    }
    ...
    if (fs.existsSync(envSource)) {
      ...
      console.log('⚠ .env.local not found - app may not work without environment variables');
    }
    ...
    console.log(`📦 Packaged app location: ${path.join(rootDir, 'dist', 'CoffeeCube-win32-x64')}`);
  } catch (error) {
