...
export const useTestMode = (): TestModeHook => {
  ...
  const cancelExecution = useCallback(async () => {
    ...
    if (globalTestConfig.debugMode) {
      console.log("[테스트 모드] 명령 실행 즉시 취소");
    }
  }, []);
    ...
    async (
      ...
      try {
        ...
        for (let i = 0; i < commands.length; i++) {
          ...
          await cancellableDelay(mockResponse.delayMs);
          ...
          if (command.receive && command.receive.trim() !== "") {
            ...
            while (!isMatched) {
              ...
              await cancellableDelay(command.duration * 1000);
              // 다음 mock 응답 가져오기 (버튼별로)
